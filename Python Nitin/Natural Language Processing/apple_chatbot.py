...
def determine_question_type(chunk):
    question_type = "yesno"
    for token in chunk:
        if token.dep_ == "amod": question_type = "info"
...
def generate_question(doc, question_type):
    ...
    doc = nlp(sent)
    for i, token in enumerate(doc):
        if token.tag_ == "PRP" and i + 1 < len(doc) and token.text == "I":
